...
const articles = [
  ...
  '../src/content/posts/ia-recouvrement-6-verifier-droits.md',
  '../src/content/posts-en/ai-debt-collection-6-data-rights.md',
...
function inspect(svg) {
  ...
  assert.equal(nodes[0].properties.viewBox, '0 0 500 350');
...
async function geometry(svg) {
  ...
  for (const node of nodes.filter(node => node.tagName === 'text')) {
    ...
    assert(size >= 20, 'Labels must remain readable at 320px');
    ...
    assert(left >= 4 && left + width <= 496 && top >= 4 && bottom <= 346, `ViewBox padding: ${label}`);
...
test('debt correction, data reuse and deadline diagrams are compact, themed, accessible and fit their own panels', async () => {
  assert.deepEqual(figures.map(set => set.length), [1, 1, 1, 1, 1, 1]);
...
test('geometry rejects a long translated label and a label shifted outside its panel', async () => {
  ...
  await assert.rejects(geometry(figures[4][0].replace('Prolongation motivée</text>', `${'W'.repeat(35)}</text>`)));
  await assert.rejects(geometry(figures[4][0].replace('x="88" y="298"', 'x="160" y="298"')));
});
