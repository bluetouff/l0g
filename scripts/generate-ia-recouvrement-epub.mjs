...
export function readingCopy(source) {
  return source
    ...
    .replace('An unfamiliar name does not prove fraud, but convincing language does not prove authenticity either.', 'Verify an unfamiliar sender independently before relying on the message or sharing sensitive information.');
}

export async function renderRecouvrementArticle(source, chapter) {
  if (!frChapters.includes(chapter) && !recouvrementChaptersEn.includes(chapter)) throw new Error('Unconfigured recouvrement chapter');
...
export async function generateRecouvrementEpub(lang = 'fr') {
  if (!['fr', 'en'].includes(lang)) throw new Error('Unsupported recouvrement edition language');
  const isEn = lang === 'en';
  const book = isEn ? recouvrementBookEn : frBook;
  const chapters = isEn ? recouvrementChaptersEn : frChapters;
  ...
  writeChapter('title_page.xhtml', book.title, `<section class="titlepage"><h1>${xml(book.title)}</h1><p class="subtitle">${xml(book.subtitle)}</p><p class="author">l0g</p><p>${tr("25 septembre 2026 · Édition française", "25 September 2026 · English edition")}</p><p>${tr("Six analyses, neuf infographies, une introduction et une conclusion originales.", "Six complete investigations, nine diagrams, an introduction and a conclusion.")}</p><p>Creative Commons Attribution 4.0 International</p></section>`, 'frontmatter');
  writeChapter('ch001.xhtml', book.introductionTitle, `<section id="introduction" epub:type="introduction"><p class="chapter-kicker">Introduction</p><h1>${xml(book.introductionTitle)}</h1><figure class="chapter-art"><img src="../media/panorama.jpg" alt="${tr("Un dossier traverse des portes de verre ; un trajet de correction revient vers les opérations.", "A paper record passes through glass gates; a correction travels back towards the operations.")}" /><figcaption>${tr("Illustration conceptuelle du parcours de lecture, créée avec une assistance d’intelligence artificielle.", "Conceptual illustration of the reading journey, created with AI assistance.")}</figcaption></figure>${paragraphs(book.introduction)}<p>${tr("Les dossiers et leurs références figurent dans les", "The investigations and their references are included in the")} <a href="../nav.xhtml#toc">${tr("six chapitres de l’enquête", "six chapters of the book")}</a>.</p></section>`);
  ...
  writeChapter('ch008.xhtml', book.conclusionTitle, `<section id="conclusion" epub:type="conclusion"><p class="chapter-kicker">Conclusion</p><h1>${xml(book.conclusionTitle)}</h1>${paragraphs(book.conclusion)}<p>${tr("Les données, les démarches et leurs délais sont détaillés et sourcés au", "Data rights, procedures and their deadlines are explained and sourced in")} <a href="ch007.xhtml#article-6">${tr("chapitre six", "chapter six")}</a>.</p></section>`);
  nav.push({ title: `${tr('Conclusion : ', 'Conclusion: ')}${book.conclusionTitle}`, href: 'text/ch008.xhtml' });
  writeChapter('ch009.xhtml', tr('À propos de cette édition', 'About this edition'), `<section id="edition"><h1>${tr("À propos de cette édition", "About this edition")}</h1><p>${tr("Cette édition du 25 septembre 2026 réunit les six volets de L’IA vous demande de payer publiés les 24 et 25 septembre 2026, leurs neuf infographies, une introduction et une conclusion originales. Les références, dates, attributions et exemples explicitement fictifs restent attachés aux textes. La relecture éditoriale a simplifié certaines formulations.", "This English edition, dated 25 September 2026, brings together the six parts of When AI Asks You to Pay published on 24 and 25 September 2026, their nine diagrams, an introduction and a conclusion. References, dates, attributions and explicitly hypothetical examples remain attached to the text. Some wording has been tightened for the book.")}</p><p>${tr("Le texte se redistribue selon l’écran. Le sommaire, les renvois de sources et les liens entre les six chapitres fonctionnent hors ligne. Les documents externes et le glossaire en ligne nécessitent une connexion. L’agrandissement des images dépend de la liseuse.", "The text reflows to fit the screen. The contents, source references and links between the six chapters work offline. External documents and the online glossary require an internet connection. Image enlargement depends on the reading app.")}</p><p>${tr("La couverture, le panorama et les six illustrations de chapitre ont été créés avec une assistance d’intelligence artificielle. Ces compositions conceptuelles accompagnent la lecture ; elles ne représentent aucun dossier personnel réel ni l’architecture d’un fournisseur cité.", "The cover, panorama and six chapter illustrations were created with AI assistance. They are conceptual compositions, without representing a real personal record or the architecture of a named provider.")}</p><p>${tr("Aucun script, police distante ou outil de suivi n’est embarqué.", "No scripts, remote fonts or tracking tools are embedded.")}</p><h2>${tr("Articles d’origine", "Original articles")}</h2><ol>${chapters.map((a) => `<li><a href="${SITE}${a.route}">${xml(a.title)}</a></li>`).join('')}</ol><p><a href="${SITE}${book.path}">${tr("Page de l’édition et mises à jour", "Edition page and updates")}</a></p><p><a href="https://creativecommons.org/licenses/by/4.0/">Creative Commons Attribution 4.0 International</a> · l0g</p></section>`);
...
<spine toc="ncx"><itemref idref="cover"/><itemref idref="title"/><itemref idref="nav"/>${sections.map((id) => `<itemref idref="${id}"/>`).join('')}</spine></package>`);
  for (const [index, name] of [[6, 'correction'], [8, tr('delais', 'deadlines')]]) copyFileSync(join(media, `file${index}.svg`), join(ROOT, 'public/publications', `${book.directory}-${tr('apercu', 'preview')}-${name}.svg`));
  console.log(`${book.title}: six complete analyses, introduction, conclusion, ${count} figures and eight illustrations.`);
}

if (process.argv[1] && resolve(process.argv[1]) === new URL(import.meta.url).pathname) {
  for (const lang of ['fr', 'en']) await generateRecouvrementEpub(lang);
}
