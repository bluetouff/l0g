...
const articles = [
  '../src/content/posts/bitget-vol-387-millions-cles-signature.md',
  '../src/content/posts-en/bitget-387-million-theft-signing-keys.md',
...
function inspect(svg) {
  ...
  assert.match(root.viewBox, /^0 0 500 342$/u);
...
test('Bitget figures keep bilingual labels within a compact themed layout', async () => {
  assert.deepEqual(figures.map(set => set.length), [2, 2]);
  for (const svg of figures.flat()) await geometry(svg);
});

test('The signing flow connects all stages and the XRP freeze path is crossed out', () => {
  for (const pair of figures) {
    const flow = inspect(pair[0]).filter(node => node.tagName === 'path').map(node => node.properties.d);
    assert.deepEqual(flow, ['M208 135H292 M285 130L292 135L285 140', 'M388 184V226 M383 219L388 226L393 219', 'M292 277H208 M215 272L208 277L215 282']);
    const freeze = inspect(pair[1]).filter(node => node.tagName === 'path').map(node => node.properties.d);
    assert.deepEqual(freeze, ['M208 128H292 M285 123L292 128L285 133', 'M208 248H236 M264 248H292', 'M243 241L257 255 M243 255L257 241']);
  }
});

test('Geometry rejects long translated labels and labels escaping their actual panel', async () => {
  await assert.rejects(geometry(figures[1][0].replace('>Secret key<', '>' + 'W'.repeat(50) + '<')));
  await assert.rejects(geometry(figures[0][1].replace('x="112.0" y="136.0"', 'x="230" y="136.0"')));
});
