...
const articles = [
  '../src/content/posts/iran-vol-teheran-dubai-sanctions-americaines.md',
  '../src/content/posts-en/iran-tehran-dubai-flight-us-sanctions.md',
...
test('Iran flight diagram preserves readable panels, theme colors and accessible names in both languages', async () => {
  assert.deepEqual(figures.map(set => set.length), [1, 1]);
  for (const svg of figures.flat()) await geometry(svg);
});

test('observed suspension and hypothetical withdrawal remain separate visual paths', () => {
  for (const [svg] of figures) {
    const paths = inspect(svg).filter(node => node.tagName === 'path');
    assert.equal(paths.length, 4);
    const observed = paths.filter(node => node.properties.stroke === 'var(--color-signal)');
    const hypothetical = paths.filter(node => node.properties.stroke === 'var(--color-accent)');
    assert.equal(observed.length, 2);
    assert.equal(hypothetical.length, 2);
    assert(observed.every(node => node.properties.strokeDashArray === undefined));
    for (const node of hypothetical) assert.deepEqual(node.properties.strokeDashArray, ['6', '5']);
    assert(observed.every(node => /130/u.test(node.properties.d)));
    assert(hypothetical.every(node => /370/u.test(node.properties.d)));
  }
});

test('geometry rejects long translations and labels crossing the boundary of their own panel', async () => {
  await assert.rejects(geometry(figures[1][0].replace('Bank / supplier', 'W'.repeat(50))));
  await assert.rejects(geometry(figures[0][0].replace('x="130.0" y="310"', 'x="235" y="310"')));
});
