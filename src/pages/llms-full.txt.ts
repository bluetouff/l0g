...
export const GET: APIRoute = async () => {
  ...
  out.push('-'.repeat(76));
  out.push(
    `Inventaire des snapshots publics et endpoints l0g : risk.json, risk-diff.json, black-box.json, debt-risk.json, confluence.json, risk-events.json, catalog.json, llms.txt, llms-full.txt, rss.xml, risk.xml, façade MCP compacte ${SITE}${MCP_COMPACT_PUBLIC_PATH} et surface MCP complète ${SITE}${MCP_PUBLIC_PATH} (Streamable HTTP, lecture seule). Precise licence CC BY 4.0, attribution, cadence best-effort et limites de fraicheur.`
  );
  out.push(`MCP compact recommandé (Streamable HTTP, six outils) : ${SITE}${MCP_COMPACT_PUBLIC_PATH}`);
  out.push(`MCP complet (Streamable HTTP, compatibilité et usages experts) : ${SITE}${MCP_PUBLIC_PATH}`);
  out.push(`Manifeste anti-dérive des outils MCP : ${SITE}/api/v1/toolset-manifest.json`);
  out.push(
    `Agent Surface v${AGENT_VERSION} : ${SITE}/donnees/agents/ documente la surface M2M statique bilingue. Chaque article expose au maximum trois claims structurants classés en fait, estimation, inférence ou scénario ; leur type heuristique reste distinct d une revue canonique. Le guide pratique ${SITE}/agents/ explique comment utiliser l0g avec un agent : choix des endpoints, prompts, citations, verification, dates separees, retrievedAt nullable, indexedAt, revue humaine et MCP public. Points d acces principaux : ${SITE}/agents.json pour la decouverte, ${SITE}/openapi.json pour le contrat, ${SITE}/api/v1/claims.json pour le graphe affirmation-source, ${SITE}/api/v1/evidence-graph.json pour le graphe articles-claims-sources, ${SITE}/api/v1/sources.json pour le registre sources, ${SITE}/api/v1/freshness.json pour la fraicheur par producteur avec sourceStatus, qualityStatus, fallbackUsed, lastAttemptAt et lastSuccessAt, ${SITE}/api/v1/risk-diff.json pour les changements de risque sur 1, 7 et 30 jours, ${SITE}/api/v1/black-box.json pour rejouer les frames point-in-time du risque, ${SITE}/api/v1/debt-risk.json pour le snapshot Dette US, ${SITE}/series/ pour le registre des séries nommées et le graphe SVG des variations, ${SITE}/api/v1/history.ndjson pour le journal operationnel brut, ${SITE}/api/v1/signals/history.json pour l historique fusionne avec evidenceTier, ${SITE}/api/v1/signals/history.ndjson pour le flux historique ligne a ligne, ${SITE}/api/v1/signals/history.csv pour les backtests pandas/R/DuckDB, ${SITE}/api/v1/integrity.json pour les empreintes SHA-256 canoniques attestees en CI et ${SITE}/api/v1/changes.json pour le changefeed machine avec objectId, version/hash courant, statut de diff et changement semantique. MCP expose aussi les resources l0g://risk-diff et l0g://black-box, les tools get_risk_diff, get_black_box et build_research_pack, ainsi que quatre prompts selectionnes par l utilisateur : audit_financial_narrative, explain_risk_change, verify_claim et replay_as_of. Les variantes NDJSON principales sont ${SITE}/api/v1/catalog.ndjson, ${SITE}/api/v1/claims.ndjson, ${SITE}/api/v1/evidence-graph.ndjson, ${SITE}/api/v1/changes.ndjson et ${SITE}/api/v1/signals/history.ndjson.`
  );
  out.push(
