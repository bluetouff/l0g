...
} from '../config/signal-freshness-contract.mjs';
...
export function buildOpenApiContract() {
  ...
  return {
    ...
    paths: {
      ...
      '/api/v1/human-traffic.json': openApiEndpoint('Lectures humaines HTML', 'GET HTML humains agrégés par jour, séparés des requêtes MCP/API, previews sociales, robots et scans, sans identifiant persistant et avec seuil k=5.', 'HumanTrafficSurface'),
    ...
    components: {
      schemas: {
        ...
        FreshnessSurface: {
          type: 'object',
          required: ['schema', 'version', 'generated', 'staleAfter', 'latest', 'corpus', 'endpoints', 'signalFreshness', 'freshnessPolicy', 'license', 'attribution'],
          additionalProperties: false,
          properties: {
            schema: { type: 'string', format: 'uri' },
            version: { type: 'string' },
            generated: { type: 'string', format: 'date-time' },
            staleAfter: { type: 'string', pattern: '^P' },
        ...
        HumanTrafficSurface: {
          type: 'object',
          required: ['schema_version', 'generated_at', 'retention_days', 'minimum_public_cohort', 'measurement', 'totals', 'daily', 'traffic_classes', 'limitations'],
          additionalProperties: false,
          properties: {
            schema_version: { const: '1.1.0' },
            generated_at: { type: 'string', format: 'date-time' },
            retention_days: { type: 'integer', const: 91 },
            minimum_public_cohort: { type: 'integer', const: 5 },
            measurement: { type: 'object', additionalProperties: { type: 'string' } },
            totals: {
              type: 'object',
              required: ['html_gets', 'days_published'],
              additionalProperties: false,
              properties: {
                html_gets: { type: 'integer', minimum: 0 },
                days_published: { type: 'integer', minimum: 0 },
              },
            },
            daily: {
              type: 'array',
              items: {
                type: 'object',
                required: ['date', 'html_gets', 'pages', 'referrers'],
                additionalProperties: false,
                properties: {
                  date: { type: 'string', format: 'date' },
                  html_gets: { type: 'integer', minimum: 5 },
                  pages: {
                    type: 'array',
                    items: {
                      type: 'object',
            ...
            traffic_classes: {
              type: 'object',
              required: ['unit', 'precedence', 'totals', 'rolling_7_days', 'definitions'],
              additionalProperties: false,
              properties: {
                unit: { type: 'string' },
                ...
                totals: {
                  type: 'object',
                  required: ['human_html', 'mcp_api', 'social_previews', 'known_crawlers', 'scans', 'other'],
                  additionalProperties: false,
                  properties: {
                    human_html: { type: 'integer', minimum: 0 },
                    mcp_api: { type: 'integer', minimum: 0 },
                    social_previews: { type: 'integer', minimum: 0 },
                    known_crawlers: { type: 'integer', minimum: 0 },
                ...
                rolling_7_days: {
                  type: 'object',
                  required: ['from', 'through', 'requests'],
                  additionalProperties: false,
                  properties: {
                    ...
                    requests: {
                      ...
                      additionalProperties: false,
                      properties: {
                        human_html: { type: ['integer', 'null'], minimum: 5 },
                        mcp_api: { type: ['integer', 'null'], minimum: 5 },
                        social_previews: { type: ['integer', 'null'], minimum: 5 },
                        known_crawlers: { type: ['integer', 'null'], minimum: 5 },
                        scans: { type: ['integer', 'null'], minimum: 5 },
                        other: { type: ['integer', 'null'], minimum: 5 },
                      },
                    },
                  },
                },
        ...
        SignalFreshnessCoverage: SIGNAL_FRESHNESS_COVERAGE_OPENAPI_SCHEMA,
        SignalFreshness: SIGNAL_FRESHNESS_OPENAPI_SCHEMA,
        FreshnessItem: {
          type: 'object',
        ...
        RiskSignal: {
          ...
          properties: {
            ...
            observedAtMethod: { const: 'latest-component-observation' },
            observationWindow: {
              type: 'object',
              required: ['oldest', 'latest'],
              additionalProperties: false,
              properties: {
            ...
            observationTimePrecision: { enum: ['date', 'date-time'] },
            observationStatus: { enum: ['known', 'missing'] },
            coverageStatus: { enum: ['complete', 'partial'] },
        ...
        DebtRiskTileSignal: {
          ...
          properties: {
            ...
            observedAt: { type: ['string', 'null'], format: 'date-time' },
            observedAtMethod: {
              anyOf: [{ const: 'latest-component-observation' }, { type: 'null' }],
            },
            observationWindow: {
              anyOf: [
                {
            ...
            observationTimePrecision: { enum: ['date', 'date-time', null] },
            observationStatus: { enum: ['known', 'missing'] },
            coverageStatus: { enum: ['complete', 'partial'] },
            missing: { type: 'array', items: { type: 'string' } },
            backtestUsable: { type: 'boolean' },
        ...
        RiskSnapshot: {
          type: 'object',
          required: ['schema', 'version', 'generated', 'snapshot', 'staleAfter', 'status', 'summary', 'indices', 'note', 'license', 'attribution'],
          additionalProperties: false,
          properties: {
            schema: { type: 'string' },
            version: { type: 'string' },
        ...
        FilingEventSource: {
          type: 'object', additionalProperties: false, required: ['accession', 'url'],
          properties: {
            accession: { type: 'string', pattern: '^[0-9]{10}-[0-9]{2}-[0-9]{6}$' },
            url: { type: 'string', format: 'uri', pattern: '^https://www[.]sec[.]gov/Archives/edgar/data/[0-9]+/[0-9]{18}/$' },
          },
        },
        FilingEventState: {
          type: 'object', additionalProperties: false,
          required: ['accession', 'form', 'published_on', 'composition_status', 'amendment_type', 'amendment_number', 'holdings_hash', 'reported_value_usd', 'portfolio_value_usd', 'positions', 'sources'],
          properties: {
            accession: { type: 'string', pattern: '^[0-9]{10}-[0-9]{2}-[0-9]{6}$' },
            form: { enum: ['13F-HR', '13F-HR/A'] },
            published_on: { type: 'string', format: 'date' },
            composition_status: { enum: ['complete', 'missing_base', 'missing_amendment', 'unknown_amendment'] },
            amendment_type: { enum: [null, 'NEW HOLDINGS', 'RESTATEMENT'] },
            amendment_number: { type: ['integer', 'null'], minimum: 1 },
            holdings_hash: { type: 'string', pattern: '^[0-9a-f]{64}$' },
            reported_value_usd: { type: ['number', 'null'], minimum: 0 },
            portfolio_value_usd: { type: ['number', 'null'], minimum: 0 },
            positions: { type: ['integer', 'null'], minimum: 0 },
            sources: { type: 'array', minItems: 1, maxItems: 100, items: { $ref: '#/components/schemas/FilingEventSource' } },
          },
        },
        FilingEvent: {
          type: 'object', additionalProperties: false,
          required: ['id', 'sequence', 'kind', 'recorded_at', 'firstSeenAt', 'fund', 'report_date', 'before', 'after', 'changed_fields'],
          properties: {
            id: { type: 'string', pattern: '^[0-9a-f]{32}$' },
            sequence: { type: 'integer', minimum: 1, maximum: 9007199254740991 },
            kind: { enum: ['baseline', 'filing_observed', 'amendment_observed', 'data_revised'] },
            recorded_at: { type: 'string', format: 'date-time' },
            firstSeenAt: { type: 'string', format: 'date-time', description: 'Première réception conservée dans le miroir l0g, borné à 100 événements.' },
            fund: {
              type: 'object', additionalProperties: false, required: ['cik', 'label'],
              properties: {
            ...
            before: { anyOf: [{ $ref: '#/components/schemas/FilingEventState' }, { type: 'null' }] },
            after: { $ref: '#/components/schemas/FilingEventState' },
            changed_fields: { type: 'array', uniqueItems: true, items: { enum: ['accession', 'form', 'published_on', 'composition_status', 'amendment_type', 'amendment_number', 'holdings_hash', 'reported_value_usd', 'portfolio_value_usd', 'positions', 'sources'] } },
          },
        },
        FilingEventFeed: {
          type: 'object', additionalProperties: false,
          description: 'Fenêtre des révisions 13F observées depuis installation. Les baselines ne reconstituent pas une connaissance historique ; les dates de publication ne sont pas des dates de transaction.',
          required: ['version', 'source', 'scope', 'status', 'lastAttemptAt', 'lastSuccessAt', 'streamId', 'startedAt', 'cursor', 'historyResetAt', 'events', 'retainedLimit'],
          properties: {
            version: { const: 1 },
            source: { const: 'https://13flow.eu/api/events/filings' },
            scope: { const: 'observed_13f_revisions' },
            status: { enum: ['ok', 'catching_up', 'unavailable'] },
            lastAttemptAt: { type: 'string', format: 'date-time' },
            lastSuccessAt: { type: ['string', 'null'], format: 'date-time' },
            streamId: { type: ['string', 'null'], pattern: '^[0-9a-f]{32}$' },
            startedAt: { type: ['string', 'null'], format: 'date-time' },
            cursor: { type: 'integer', minimum: 0, maximum: 9007199254740991 },
            historyResetAt: { type: ['string', 'null'], format: 'date-time' },
            producerRevision: { type: ['string', 'null'], pattern: '^[0-9a-f]{40}$' },
            upstreamGeneratedAt: { type: ['string', 'null'], format: 'date-time' },
            retainedLimit: { const: 100 },
            reason: { type: 'string', maxLength: 240 },
            events: { type: 'array', maxItems: 100, items: { $ref: '#/components/schemas/FilingEvent' } },
          },
        },
        RiskConfluence: {
          type: 'object',
          required: ['generated', 'updated', 'retrievedAt', 'lastAttemptAt', 'lastSuccessAt', 'sourceStatus', 'qualityStatus', 'fallbackUsed', 'fallbackReason', 'staleAfter', 'ageSeconds', 'timelinessStatus', 'provenanceStatus', 'freshness', 'count', 'conviction', 'top', 'source', 'note'],
          additionalProperties: false,
          properties: {
            ...
            lastAttemptAt: { type: ['string', 'null'], format: 'date-time' },
            lastSuccessAt: { type: ['string', 'null'], format: 'date-time' },
            sourceStatus: { enum: ['ok', 'fallback', 'failed', 'unknown'] },
            qualityStatus: { enum: ['nominal', 'limited', 'degraded', 'unknown'] },
            fallbackUsed: { type: 'boolean' },
            fallbackReason: { type: ['string', 'null'] },
            staleAfter: { type: 'string', pattern: '^P' },
            ageSeconds: { type: ['integer', 'null'] },
            timelinessStatus: { enum: ['fresh', 'stale', 'unknown'] },
            provenanceStatus: { enum: ['verified', 'partial', 'unverified'] },
            freshness: { type: ['object', 'null'] },
            filingEvents: { anyOf: [{ $ref: '#/components/schemas/FilingEventFeed' }, { type: 'null' }] },
            count: { type: 'integer' },
...
export function buildFreshnessSurface(posts: PostEntry[], guides: GuideEntry[], risk?: RiskSnapshotInput | null) {
  ...
  return {
    ...
    endpoints: [
      ...
      { path: '/api/v1/human-traffic.json', role: 'Lectures humaines HTML agrégées sans identifiant', update: 'chaque jour' },
...
export function buildAgentManifest(posts: PostEntry[], guides: GuideEntry[]) {
  return {
    ...
    endpoints: {
      ...
      humanTraffic: `${AGENT_SITE}/api/v1/human-traffic.json`,
      llms: `${AGENT_SITE}/llms.txt`,
      llmsFull: `${AGENT_SITE}/llms-full.txt`,
      llmsFullEn: `${AGENT_SITE}/llms-full-en.txt`,
      mcpEndpoint: `${AGENT_SITE}${MCP_COMPACT_PUBLIC_PATH}`,
      mcpCompactEndpoint: `${AGENT_SITE}${MCP_COMPACT_PUBLIC_PATH}`,
      mcpFullEndpoint: `${AGENT_SITE}${MCP_PUBLIC_PATH}`,
      mcpDocumentation: `${AGENT_SITE}/mcp/`,
      docs: `${AGENT_SITE}/donnees/agents/`,
      agentBenchDocumentation: `${AGENT_SITE}/agent-bench/`,
    },
    preferredUse: [
      'Commencer les intégrations MCP par /api/mcp/compact et appeler get_risk_state pour toute question sur l’état du risque.',
      'Réserver /api/mcp à la recherche experte nécessitant les tools spécialisés.',
